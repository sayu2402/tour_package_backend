...
class TourPackageAdminSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        rep["source_country"] = (
            {"id": instance.source_country.id, "name": instance.source_country.name}
            if instance.source_country
            else None
        )
        ...
        rep["source_city"] = (
            {"id": instance.source_city.id, "name": instance.source_city.name}
            if instance.source_city
            else None
        )

        rep["destination_city"] = (
            {"id": instance.destination_city.id, "name": instance.destination_city.name}
            if instance.destination_city
            else None
        )
...
class EnquirySerializer(serializers.ModelSerializer):
    ...
    def get_related_schedule_title(self, obj):
        print(
            f"Checking enquiry ID {obj.id} - related_schedule: {obj.related_schedule}"
        )
